...
// dado el operador (+, -, *, /) retorna la operación aritmética
...
long eval(mpc_ast_t* t) {
    // si el tag del ast es "number" retornamos el número de una
    if (strstr(t->tag, "number")) {
        return atoi(t->contents); // intentamos convertir a número
    }
    // el operador está siempre en la posición 1
    char* op = t->children[1]->contents;

    // almacenamos el tercer hijo en `x`
    long x = eval(t->children[2]);

    // iteramos sobre los hijos restantes y combinamos
    int i = 3;
